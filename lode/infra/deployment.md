...
- **The QR scanner library is code-split.** `jsqr` is loaded via a dynamic `import('jsqr')` inside `initScanner` (`Pages.fs`), so Vite emits it as a separate chunk that is only downloaded when the user opens the scanner — it is kept out of the main bundle that every first load pays for.
