...
- **The QR scanner library is code-split.** `jsqr` is loaded via a dynamic `import('jsqr')` inside `initScanner` (`src/Client/Pages/Common.fs`), so Vite emits it as a separate chunk that is only downloaded when the user opens the scanner — it is kept out of the main bundle that every first load pays for.
- **Hashed bundle assets are cached immutably.** Vite content-hashes everything under `/assets/`, so the reverse proxy serves it with `Cache-Control: public, max-age=31536000, immutable` (matching the photo caching in `Program.fs`). `index.html` is served `Cache-Control: no-cache` so new deploys are picked up immediately. Configured in both `deploy/Caddyfile` (live) and `deploy/nginx.conf` (legacy). This removes a per-asset revalidation round trip on every full load — significant on a latency-bound VPS.
